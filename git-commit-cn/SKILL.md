...
无论用户使用中文还是英文描述变更内容,提交信息一律使用中文输出。这是本 skill 的核心目标——确保项目 git 历史中的提交信息始终保持中文一致性。
...
将相关变更分组提交,每个提交只做一件事。判断是否需要拆分提交的原则:

- **同一个功能/修复的变更** → 合为一个提交,即使涉及多个文件(如代码 + 测试 + 文档)
- **不同目的的变更** → 拆分为多个提交(如一个 feat + 一个 fix 不应混在一起)
- **大型重构** → 如果能按模块拆分且各部分独立可用,建议拆分;如果拆开后代码无法正常工作,保持一个提交
