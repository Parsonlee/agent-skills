...
| Domain | Command | Why |
|--------|---------|-----|
| `mp.weixin.qq.com` | `uv run fetch.py <url> --stealth` | JS-rendered content |
| `zhuanlan.zhihu.com` | `uv run fetch.py <url> --stealth` | Anti-scraping + JS |
| `juejin.cn` | `uv run fetch.py <url> --stealth` | JS-rendered SPA |
| `sspai.com` | `uv run fetch.py <url>` | Static HTML |
| `blog.csdn.net` | `uv run fetch.py <url>` | Static HTML |
| `ruanyifeng.com` | `uv run fetch.py <url>` | Static blog |
| `openai.com` | `uv run fetch.py <url>` | Static HTML |
| `blog.google` | `uv run fetch.py <url>` | Static HTML |
| Everything else | `uv run fetch.py <url>` | Auto-fallback handles it |
...
This skill uses [uv](https://docs.astral.sh/uv/) for zero-config dependency management. No manual `pip install` is required; `uv` will automatically handle dependencies on first run.
